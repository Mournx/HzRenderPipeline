...
namespace HzRenderPipeline.Runtime {
  public  class HzRenderPipeline : RenderPipeline
  {
    ...
    private Matrix4x4 prevfrustumCornersWS;
    private Vector2 curJitter;
    private Vector2[] jitterPatterns = new Vector2[8];
    private Matrix4x4 nonjitterVPMatrix;
    private Matrix4x4 nonjitterVPMatrixInv;
    private Matrix4x4 vpMatrix;
    private Matrix4x4 vpMatrixInv;
    private Matrix4x4 vpMatrixPrev; // nonjittered
    private Matrix4x4 vpMatrixInvPrev; // nonjittered
    ...
    protected override void Render(ScriptableRenderContext context, Camera[] cameras)
    {
      ...
      nonjitterVPMatrix = GL.GetGPUProjectionMatrix(camera.nonJitteredProjectionMatrix, false) * viewMatrix;
      nonjitterVPMatrixInv = nonjitterVPMatrix.inverse;


      prevfrustumCornersWS = frustumCornersWS;
      frustumCornersWS = new Matrix4x4();

      var cameraViewMatrix = camera.worldToCameraMatrix;
      Vector3 cameraFwdWS = -cameraViewMatrix.GetRow(2);
      Vector3 cameraUpWS = cameraViewMatrix.GetRow(1);
      Vector3 cameraRightWS = cameraViewMatrix.GetRow(0);
      var farHalfFovTan = camera.farClipPlane * Mathf.Tan(0.5f * Mathf.Deg2Rad * camera.fieldOfView);
      var fwdDir = cameraFwdWS * camera.farClipPlane;
      var upDir = cameraUpWS * farHalfFovTan;
      var rightDir = cameraRightWS * farHalfFovTan * camera.aspect;
      var topLeft = fwdDir + upDir - rightDir;
      var topRight = fwdDir + upDir + rightDir;
      var bottomLeft = fwdDir - upDir * 3f - rightDir;
      frustumCornersWS.SetRow(0, new float4(topLeft, .0f));
      frustumCornersWS.SetRow(1, new float4(bottomLeft, .0f));
      frustumCornersWS.SetRow(2, new float4(topRight, .0f));
      frustumCornersWS.SetRow(3, new float4((camera.farClipPlane - camera.nearClipPlane) / camera.nearClipPlane, 1f,
        (camera.farClipPlane - camera.nearClipPlane) / (camera.nearClipPlane * camera.farClipPlane),
        1f / camera.farClipPlane));
      ...
      Shader.SetGlobalMatrix("_nonjitterVPMatrix", nonjitterVPMatrix);
      Shader.SetGlobalMatrix("_nonjitterVPMatrixInv", nonjitterVPMatrixInv);
      Shader.SetGlobalMatrix("_FrustumCornersWS", frustumCornersWS);
      ...
      /*//设置TAA
      var jitterNum = (int)taaSettings.jitterNum;
      ...
      Shader.SetGlobalVector("_FrameParams", frameParams);

      curJitter = jitterPatterns[frameNumCycled];
      curJitter *= taaSettings.jitterSpread;
      var taaJitter = new Vector4(curJitter.x, curJitter.y, curJitter.x / Screen.width, curJitter.y / Screen.height);
      Shader.SetGlobalVector("_JitterParams", taaJitter);
      if (taaSettings.enabled) ConfigureProjectionMatrix(camera, curJitter);
