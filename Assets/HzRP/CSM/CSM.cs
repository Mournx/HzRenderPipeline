...
public class CSM
{
    ...
    Vector3[] lightSpaceOBB(Vector3[] nearCorners, Vector3[] farCorners, Vector3 lightDir)
    {
        ...
        Vector3[] obbCorners =
        {
            ...
            new Vector3(min.x, max.y, max.z), new Vector3(max.x, min.y, min.z), new Vector3(max.x, min.y, max.z), new Vector3(max.x, max.y, min.z), max
        };
    ...
    public void Update(Camera mainCam, Vector3 lightDir, CSMSettings csmSettings)
    {
        //获取主相机视锥体
        mainCam.CalculateFrustumCorners(new Rect(0, 0, 1, 1), mainCam.nearClipPlane, Camera.MonoOrStereoscopicEye.Mono, nearCorners);
        mainCam.CalculateFrustumCorners(new Rect(0, 0, 1, 1), csmSettings.maxDistance, Camera.MonoOrStereoscopicEye.Mono, farCorners);
    ...
    public void ConfigCameraToShadowSpace(ref Camera camera, Vector3 lightDir, int level, int resolution)
    {
        ...
        float len = 0;
        if (level == 0) { box = box0; len = Vector3.Magnitude(f0_far[2] - f0_near[0]); }
        if (level == 1) { box = box1; len = Vector3.Magnitude(f1_far[2] - f1_near[0]); }
        if (level == 2) { box = box2; len = Vector3.Magnitude(f2_far[2] - f2_near[0]); }
        if (level == 3) { box = box3; len = Vector3.Magnitude(f3_far[2] - f3_near[0]); }
